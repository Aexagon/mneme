...
The Mneme plugin: a SessionStart hook that loads a persistent cache into every chat, a SessionEnd distiller that auto-captures learnings to the inbox, five commands for the learning loop, and two skills.
...
Cache data lives OUTSIDE this plugin: `~/.claude/mneme/cache/` (global), `<project>/.mneme/cache/` (project), and the auto-capture inbox `~/.claude/mneme/inbox/`. Updating or reinstalling the plugin never touches it.
