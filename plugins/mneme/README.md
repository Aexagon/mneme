...
The Mneme plugin: a SessionStart hook that loads a persistent cache into every chat, three commands for the learning loop, and two skills.

- `hooks/load-cache.sh` — injects the cache index (global + project overlay) + the loop protocol at session start.
